...
export default function Viz(model, controller) {
	...
	function draw_services_statuses() {
		...
		d3.selectAll(".node").each(function () {
			...
			for (let i=0; i<model.services.length; i++) {
				const isUp = model.services[i].visible && nData.servicesStatuses[i];
				hasUp = hasUp || isUp;
				n.select(".service_" + i).attr("fill", isUp ? model.services[i].color : DOWN_SERVICE_COLOR);
			}
			if (hasUp)
				teams_with_alive++;
		});
	...
	function updateOpenedTooltip() {
		if (openedTooltipTeamId == undefined)
			return;
		const team = model.teams[openedTooltipTeamId];
	...
	function setOptimalZoom() {
		...
		const cad = getCetnerAndDelta(model.teams);
	...
	function drawTeams() {
		const coordsForTeams = getCoordsForTeams(model.teams.length);
		...
		for (let i=0; i<model.services.length; i++) {
			...
			$filter.click( function(index) {return function () {
				if ($(this).hasClass(deselectionFlag)) {
					...
					model.services[index].visible = true;
					draw_services_statuses();
				} else {
					$(this).addClass(deselectionFlag);
					model.services[index].visible = false;
					draw_services_statuses();
				}
			};
			}(i));
